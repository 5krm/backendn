...
use App\Http\Controllers\Api\Mobile\MobileCertificateController;
...
use App\Http\Controllers\Api\Mobile\Tutor\MobileTutorProfileController;
...
Route::prefix('mobile/v1')->name('mobile.')->middleware(['check_api_version', 'api_locale', 'throttle:mobile'])->group(function () {
    ...
    Route::post('certificates/verify', [MobileCertificateController::class, 'verify'])->name('certificates.verify');

    // Tutor public profile
    Route::get('tutors/{id}', [MobileTutorProfileController::class, 'show'])->name('tutors.show');
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        Route::prefix('lessons/{lesson}')->name('lessons.')->group(function () {
            ...
            Route::post('comments', [MobileCommentsController::class, 'store'])->middleware(FilterProfanity::class)->name('comments.store');
        });

        // Certificate
        Route::get('courses/{course}/certificate', [MobileProgressController::class, 'getCertificate'])->name('certificate');
        Route::post('certificates/generate', [MobileCertificateController::class, 'generate'])->name('certificates.generate');

        // Leaderboard
        Route::get('leaderboard', [MobileLeaderboardController::class, 'index'])->name('leaderboard.index');
